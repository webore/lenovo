...
int s3cfb_mipi_clk_on(void)
{
	...
	rate = clk_round_rate(sclk, 800000000);
...
int s3cfb_clk_on(struct platform_device *pdev, struct clk **s3cfb_clk)
{
	...
	if (soc_is_exynos4412()) {
		...
		if (samsung_rev() >= EXYNOS4412_REV_2_0){
			mout_mpll_rate = clk_get_rate(mout_mpll);
			if(mout_mpll_rate == 880000000)
			{
				mout_mpll = clk_get(&pdev->dev, "mout_epll");//if uboot set MPLL 880M, change fimd0 clk EPLL_400
				printk("============ lcd using EPLL400\n");
			}else
				printk("============ lcd using MPLL800\n");
		}
	}
	...
	if ((soc_is_exynos4412()) && (samsung_rev() >= EXYNOS4412_REV_2_0)){
		if(mout_mpll_rate == 880000000)
			ret = clk_set_rate(sclk, 400000000);  //if uboot set MPLL 880M, change fimd0 clk EPLL_400
		else
			ret = clk_set_rate(sclk, 800000000);  //if uboot set MPLL 800M, keep fimd0 clk MPLL_800
	}
	else

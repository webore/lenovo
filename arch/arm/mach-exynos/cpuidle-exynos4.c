...
static int check_usb_op(void)
{
	...
	/*diog.zhao 
	*Usb is working, can not enter into lpa mode.
	*/
...
#endif
#if defined(CONFIG_MACH_STUTTGART) && defined(CONFIG_BT_HCIUART_PS)
extern int check_bt_status(void); /*jeff, by bluesleep driver */
#endif

#ifdef CONFIG_MACH_STUTTGART
extern int check_gps_status(void);
#endif
...
#if defined(CONFIG_MACH_STUTTGART) && defined(CONFIG_BT_HCIUART_PS)
	if (check_bt_status())
		return 1;  /*jeff, by bluesleep driver */
#endif
